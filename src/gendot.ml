...
let new_node s g label = new_generic_node s g [
  `Label label;
  `Shape `Box;
  `Style `Filled;
  `Fillcolor 0x00EEEEEE;
]

let new_dummy_node s g label = new_generic_node s g [
  `Label label; 
  `Shape `Box;
  `Style `Dashed;
  `Fillcolor 0x00777777;
]

let new_literal_node s g label = new_generic_node s g [
  `Label label; 
  `Shape `Box;
  `Style `Filled;
  `Fillcolor 0x0042D7F4;
]

let new_ident_node s g label = new_generic_node s g [
  `Label label; 
  `Shape `Box;
  `Style `Filled;
  `Fillcolor 0x00F9F0A7;
]

let new_op_node s g label = new_generic_node s g [
  `Label label; 
  `Shape `Box;
  `Style `Filled;
...
let rec dot_expr s g = function
  ...
  | Ast.UnExpr (op, rhs) ->
    ...
    G.add_edge g op_node rhs_node ;
    op_node
  | Ast.BoolLiteral b -> new_literal_node s g (string_of_bool b)
  | Ast.IntLiteral i -> new_literal_node s g (string_of_int i)
