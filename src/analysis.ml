...
let check_const_decl env {Ast.cd_name; cd_value} = 
  if Env.constant_exists cd_name env then
    ...
    | Ast.BoolLiteral b -> 
        Ok (Env.add_constant cd_name {loc=cd_value.loc; value=(Env.Bool b)} env, [])
    | Ast.IntLiteral i -> 
        Ok (Env.add_constant cd_name {loc=cd_value.loc; value=(Env.Int i)} env, [])
    | Ast.FloatLiteral f -> 
        Ok (Env.add_constant cd_name {loc=cd_value.loc; value=(Env.Float f)} env, [])
    | _ -> 
...
let check_array_dim env = function
  ...
  | Type.OfName name -> (
      ...
      | Some {value=(Env.Int _); _} -> Ok env
      | Some {value=(Env.Bool _); _} -> Error (`NonIntegerArraySize)
      | Some {value=(Env.Float _); _} -> Error (`NonIntegerArraySize)
...
let rec check_type ctx_loc env = function
  ...
  | Type.Record fields ->
      List.fold_left
        (fun env f -> 
          env >>= fun env -> check_type ctx_loc env f.Type.t)
        (Ok env)
        fields
  | Type.Array (t, dims) ->
      check_type ctx_loc env t >>= fun env ->
      (List.fold_left
        (fun env dim ->
  ...
  | Type.Function (args, rets) ->
      ...
      (List.fold_left
        (fun env f -> 
          env >>= fun env -> 
          check_type ctx_loc env f.Type.t >>= fun env ->
          (Ok (Env.add_var f.name {loc=ctx_loc; value=f.t} env)))
        (Ok env)
        args) >>= fun env ->
      (List.fold_left
...
let check_type_decl ctx_loc env {Ast.td_name; td_type} = 
  if Env.type_exists td_name env then
    ...
    | Record fields ->
        ...
        check_unique ids errfn >>= fun _ ->
        check_type ctx_loc env td_type >>= fun _ ->
...
let check_pipeline_functions env functions =
  ...
  List.fold_left
    (fun acc {Located.loc; value=fdecl} ->
      ...
      Ok (Env.add_function fdecl.fd_name {Located.loc=loc; value=fdecl.fd_type} env, f :: funcs))
    (Ok (env, []))
    functions
...
let check_pipeline_decl ctx_loc env {Ast.pd_name; pd_type; pd_functions} = 
  if Env.pipeline_exists pd_name env then
    ...
    Ok (Env.add_pipeline pd_name {loc=ctx_loc; value=pd_type} env) >>= fun genv ->
    check_type ctx_loc env pd_type >>= fun env ->
...
let check_toplevel env {Located.loc; value} = match value with 
  | Ast.ConstDecl cd -> check_const_decl env cd
  | Ast.TypeDecl td -> check_type_decl loc env td
  | Ast.PipelineDecl pd -> check_pipeline_decl loc env pd
